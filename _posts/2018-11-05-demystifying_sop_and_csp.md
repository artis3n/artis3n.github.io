...
Note that scripts executed from pages with an `about:blank` or `javascript:` URL inherit the origin of the document that opened the URL. This means that [Cross-Site Scripting][xss] (XSS) will execute with the inherited origin of the page it is running on. Also, code that opens a new tab/window can then write content into it, as the new tab/window inherits the origin of the page that created it. The former is bad. The latter can be "business as usual," but is often abused by malicious ads and adware (ALERT! YOUR COMPUTER HAS 37 VIRUSES CLICK HERE FOR A FREE SCAN!).
